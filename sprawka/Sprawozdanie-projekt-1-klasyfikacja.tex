...
\begin{itemize}
    \item $k = 13$
    \item wielkość zbioru testowego = 15\%
    \item metryka = euklidesowa
    \item liczba cech = 11
    \item Accuracy = 0,797677
\end{itemize}
...
\begin{enumerate}
	\item alnumToOtherCharsRatio, charCount, longWordToOtherWordsRatio, \\
	properNounMidSentenceRatio, properNounFirst, rarestRepeatedWord, \\
	upperToAllCharsRatio, upperToLowerRatio, wordCount (9 cech)
	\item alnumToOtherCharsRatio, charCount, longWordToOtherWordsRatio, \\
	properNounFirst, rarestRepeatedWord, upperToAllCharsRatio, \\
	upperToLowerRatio, wordCount (8 cech)
	\item alnumToOtherCharsRatio, charCount, longWordToOtherWordsRatio, \\
	...
	\item alnumToOtherCharsRatio, charCount, longWordToOtherWordsRatio, \\
	properNounMidSentenceRatio, properNounFirst, rarestRepeatedWord, \\
	upperToAllCharsRatio, wordCount (8 cech)
\end{enumerate}
...
\noindent
Najlepszy wynik został osiągnięty dla następujących parametrów:
\begin{itemize}
	\item $k = 13$
	\item wielkość zbioru testowego = 15\%
	\item metryka = euklidesowa
	...
	\item zestaw cech: 
	\begin{itemize}
		\item alnumToOtherCharsRatio
		\item charCount
		\item longWordToOtherWordsRatio
		\item properNounFirst
		\item rarestRepeatedWord
		\item upperToAllCharsRatio
		\item upperToLowerRatio
		\item wordCount
	\end{itemize}
\end{itemize}
...
\begin{table}[H]
	\centering
	\caption{Wyniki klasyfikacji dla najlepszego modelu}
	\label{tab:classification_report}
	\begin{tabular}{lccc}
		\hline
		\textbf{Klasa} & \textbf{Precision} & \textbf{Recall} & \textbf{F1-score} \\
		\hline
		france         & 0.2500 & 0.0244 & 0.0444 \\
		japan          & 0.6250 & 0.1333 & 0.2198 \\
		uk             & 0.5000 & 0.2057 & 0.2915 \\
		canada         & 0.4219 & 0.2126 & 0.2827 \\
		usa            & 0.8271 & 0.9730 & 0.8941 \\
		west-germany   & 0.5000 & 0.0400 & 0.0741 \\
		\hline
		\multicolumn{4}{l}{Accuracy $\approx$ 0.8020} \\
		\multicolumn{4}{l}{Macro F1-score $\approx$ 0.3011} \\
		\hline
	\end{tabular}
\end{table}

\noindent
Model osiąga wysoką dokładność ogólną, jednak wyniki dla poszczególnych klas są silnie zróżnicowane. Bardzo dobra skuteczność dla klasy USA znacząco zawyża wartość Accuracy, podczas gdy dla pozostałych klas model radzi sobie wyraźnie gorzej. Niska wartość macro F1-score wskazuje na brak równowagi w jakości klasyfikacji między klasami. Otrzymane wyniki sugerują, że model ma ograniczoną zdolność do poprawnego rozróżniania mniej reprezentowanych kategorii.
...
\newpage
	...
	\subsection{Przygotowanie zbioru danych i selekcja klas}

	Ograniczenie liczby etykiet w analizowanej kategorii prowadzi do wyraźnej poprawy działania algorytmu klasyfikacji. Mniejsza liczba klas zmniejsza złożoność problemu, ogranicza możliwość popełniania błędów oraz pozwala modelowi lepiej rozróżniać dostępne kategorie. Dzięki temu uzyskiwane wyniki są bardziej trafne i stabilne.

	\subsection{Metryki odległości oraz miara podobieństwa tekstów}

	W procesie klasyfikacji warto rozważyć metryki takie jak euklidesowa i Manhattan oraz odpowiednio dobraną miarę podobieństwa tekstów. Metryka Czebyszewa nie sprawdza się w tym zadaniu i nie powinna być stosowana, ponieważ nie oddaje dobrze różnic między tekstami, co pogarsza wyniki klasyfikacji.
	
	\subsection{Miary jakości klasyfikacji}

	Accuracy jest użyteczne jako ogólny wskaźnik poprawności, jednak nie powinno być jedynym kryterium oceny. W przypadku niezrównoważonych danych (np. gdy jedna klasa dominuje) może ono dawać myląco dobre wyniki. Dlatego konieczne jest uwzględnienie miary F1, która lepiej odzwierciedla rzeczywistą jakość klasyfikacji, łącząc Precision i Recall.

	W naszym przypadku optymalizowaliśmy parametry sprawdzając tylko wartość Accuracy, co w przypadku gdy dominuje jedna klasa spowodowało, że inne, mniej liczne klasy osiągały niższe wyniki F1. 
		
	\subsection{Wpływ parametru $k$ na skuteczność klasyfikacji}

	Parametr k był badany do czasu aż nie spadnie trzykrotnie pod rząd dla danej metryki.
	Wyniki eksperymentu widoczne na wykresie \ref{fig:exp1}.

	Najlepsze wyniki uzyskano dla wartości parametru k w zakresie 10-13. W tym przedziale accuracy wynosiło około 79,5\% dla metryk euklidesowej i Manhattan oraz około 77,5\% dla metryki Czebyszewa, która osiągała wyraźnie słabsze rezultaty.

	\subsection{Wpływ proporcji podziału zbioru na jakość klasyfikacji}

	Procentowy udział zbioru testowego analizowano w zakresie od 10\% do 90\%, z krokiem równym 5\%.
	Wyniki eksperymentu widoczne na wykresie \ref{fig:exp2}.

	Najlepsze wyniki uzyskano przy 15\% danych przeznaczonych na zbiór testowy. W przypadku bardzo małego zbioru testowego wyniki są stosunkowo dobre, jednak nie w pełni miarodajne. Natomiast wraz ze wzrostem udziału zbioru testowego obserwuje się wyraźny spadek accuracy, przy dużych proporcjach podziału wyniki stają się wyraźnie gorsze i mało satysfakcjonujące.

	\subsection{Wpływ wyboru metryki i miary podobieństwa na wyniki klasyfikacji}

	Parametr k był badany do czasu aż nie spadnie trzykrotnie pod rząd dla danej metryki.
	Wyniki eksperymentu widoczne na wykresie \ref{fig:exp1}.

	Najlepsze wyniki klasyfikacji uzyskano dla metryki euklidesowej, która osiągała najwyższe wartości Accuracy. W porównaniu do niej metryka Manhattan dawała zbliżone, lecz nieco słabsze rezultaty, natomiast metryka Czebyszewa wypadała najgorzej i nie powinna być preferowana w tym zadaniu.

	\subsection{Analiza wpływu cech na wyniki klasyfikacji}

	Analizowano wszystkie możliwe kombinacje cech o liczności od 8 do 11, przy czym początkowy zbiór obejmował 11 cech.
	Wyniki eksperymentu widoczne na wykresie \ref{fig:exp3}.

	Na podstawie analizy czterech różnych podzbiorów cech można zauważyć, że największy wpływ na wyniki klasyfikacji (w szczególności na accuracy) ma cecha rarestRepeatedWord. Osiąga ona najlepsze rezultaty spośród rozważanych wariantów, jednak przewaga ta jest bardzo niewielka - wynosi jedynie około 0,2\% w stosunku do pozostałych cech.
	Pozostałe cechy wypadają praktycznie na tym samym poziomie, co sugeruje, że ich wpływ na skuteczność klasyfikacji jest zbliżony i żadna z nich nie wyróżnia się znacząco ani na plus, ani na minus.
