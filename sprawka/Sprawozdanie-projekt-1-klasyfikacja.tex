...
\begin{document}
	...
	\subsection{Metryki cząstkowe per klasa}
	...
	\subsection{Metryki globalne}
	Do globalnej oceny modelu zastosowano:
	\begin{enumerate}
		\item \textbf{Accuracy $Acc$}: Stosunek wszystkich poprawnych klasyfikacji do liczby wszystkich dokumentów $M$.
		\begin{equation}
	...
	\begin{table}[h!]
		...
		\begin{tabular}{|l|c|c|c|}
			\hline
			\textbf{Klasa / Metryka} & \textbf{Precision (\(P_i\))} & \textbf{Recall (\(R_i\))} & \textbf{F1-score (\(F1_i\))} \\ \hline
			usa & 82,75\%  & 94,81\% & 88,37\% \\ \hline
			canada & 53,06\% & 15,29\% & 23,74\% \\ \hline 
			japan & 34,38\% & 22,00\% & 26,83\% \\ \hline \hline
			
			\textbf{Metryka globalna} & \multicolumn{2}{c|}{\textbf{Nazwa}} & \textbf{Wartość} \\ \hline
	...
	Algorytm osiągnął dokładność bliską \(79\%\), jednak łatwo zauważyć niskie wartości metryk dla krajów innych niż USA, wynika to z znacznej przewagi USA w danych. Metryk Macro-F1 znacznie lepiej odzwierciedla słabości modelu w rozpoznawaniu innych klas.
